...
public class IntegersTest {
	...
	public void testShortFromInts() {		
		assertEquals(0x1020, Integers.shortFromInts(new int[]{0x10,0x20}, 0, 1));
		assertEquals(0x2010, Integers.shortFromInts(new int[]{0x10,0x20}, 1, 0));
		assertEquals((short) 0x8000, Integers.shortFromInts(new int[]{0x80,0x00}, 0, 1));
		assertEquals(0x8000, (int) Integers.shortFromInts(new int[]{0x80,0x00}, 0, 1) & 0xFFFF);
		assertEquals(0x0080, Integers.shortFromInts(new int[]{0x80,0x00}, 1, 0));
	}

	@Test
	public void testLongFromInts() {		
		assertEquals(0x1020304050607080L, Integers.longFromInts(
			new byte[]{0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, (byte) 0x80}, 0, 7
		));
		assertEquals(0x8070605040302010L, Integers.longFromInts(
				new byte[]{0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, (byte) 0x80}, 7, 0
		));
		assertEquals(0x1020304050607080L, Integers.longFromInts(
				new int[]{0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80}, 0, 7
			));
		assertEquals(0x8070605040302010L, Integers.longFromInts(
				new int[]{0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80}, 7, 0
		));		
	}
	...
	public void testReadWriteObject() {		
		...
		Comparable[] original = new Comparable[]{
			//from 1-st to 4-th
			Boolean.TRUE, Boolean.FALSE, new Byte((byte) 1), new Byte((byte) -1), 
			//from 5-th to 8-th
			new Short((short) 1), new Short((short) -1), new Short((short) 256), new Short((short) -256),  
			//from 9-th to 14-th
			new Integer(1), new Integer(-1), new Integer(256), new Integer(-256), new Integer(65536), new Integer(-65536),
			//from 15-th to 22-nd
			new Long(1), new Long(-1), new Long(256), new Long(-256), new Long(65536), new Long(-65536), new Long(4294967296l), new Long(-4294967296l) 
		};
		...
		for (int i = 0; i < readback.length; i++) {
			...
			idx += Integers.readObject(buffer, idx, original[i].getClass(), aux);
			readback[i] = aux[0];
		}
		
		for (int i = 0; i < original.length; i++) {
			boolean result = original[i].compareTo(readback[i]) ==  0;
			assertTrue("Camparison "+(i+1)+"-th item of original[] with readback[] of type "
					+original[i].getClass().getName()+" original was "+original[i]+", but read "+readback[i], result);
			assertEquals("Camparison "+(i+1)+"-th item of original[] with readback[] of type "
					+original[i].getClass().getName(), original[i].toString(),readback[i].toString());			
		}
	...
	public void testGetByteAsInteger() {
		int[] data; long l; short s;
		
		data = new int[]{0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80};
		l = 0x8080808080808080L; 
		for (int i = 0; i < data.length; i++) {
			assertEquals("Comparing the "+(i+1)+"-th byte",data[i],Integers.getByteAsInteger(l, i));
		}		
		...
		s = (short) 0x8080;
		for (int i = 0; i < data.length; i++) {
			assertEquals("Comparing the "+(i+1)+"-th byte",data[i],Integers.getByteAsInteger(s, i));
		}
	...
	public void testGetByte() {
		byte[] data; long l; int y; short s;
		
		data = new byte[]{(byte) 0x80, (byte) 0x80, (byte) 0x80, (byte) 0x80, (byte) 0x80, (byte) 0x80, (byte) 0x80, (byte) 0x80};
		l = 0x8080808080808080L; 
		for (int i = 0; i < data.length; i++) {
			assertEquals("Comparing the "+(i+1)+"-th byte",data[i],Integers.getByte(l, i));
		}		
		
		data = new byte[]{0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, (byte) 0x80};
		l = 0x8070605040302010L;
		for (int i = 0; i < data.length; i++) {
			assertEquals("Comparing the "+(i+1)+"-th byte",data[i],Integers.getByte(l, i));
		}
		...
		y = 0x80808080; 
		for (int i = 0; i < data.length; i++) {
			assertEquals("Comparing the "+(i+1)+"-th byte",data[i],Integers.getByte(y, i));
		}		
		...
		s = (short) 0x8080;
		for (int i = 0; i < data.length; i++) {
			assertEquals("Comparing the "+(i+1)+"-th byte",data[i],Integers.getByte(s, i));
		}
	...
	public void testReadWriteInt() {
		int[] expected = new int[]{
			0 , 1, -1, 256, -256, 65793, -65793, 16843009, -16843009	
		};
